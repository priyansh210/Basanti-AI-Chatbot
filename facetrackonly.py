...
# if not os.path.isdir('./face regonition'):
#     os.makedirs('./face regonition')
# if not os.path.isdir('./face regonition/ph'):
#     os.makedirs('./face regonition/ph')
...
def identify_face(facearray):
    model = joblib.load(r".\face regonition\face_recognition_model.pkl")
    return model.predict(facearray)
# A function which trains the model on all the faces available in faces folder
def train_model():
    faces = []
    labels = []
    userlist = os.listdir('face regonition/ph/')
    for user in userlist:
        for imgname in os.listdir(f'face regonition/ph/{user}'):
            img = cv2.imread(f'face regonition/ph/{user}/{imgname}')
    ...
    joblib.dump(knn, './face regonition/face_recognition_model.pkl')
...
def detect_face():
    if 'face_recognition_model.pkl' not in os.listdir(r".\face regonition"):
...
def add_face():
    ...
    userimagefolder = './face regonition/ph/'+newusername+'_'+newuserrole+'_'+str(newuserid)
